...
const NEGATIVE_SCAN_COLOR = "#FF0099";
...
export default function ScanGridButton(props: Props) {
    ...
    const { color: scanColor = "#00FF66", speed: speedPctProp } = scan;
    const hoverScanColor = sameColor(scanColor, hoverFill)
        ? NEGATIVE_SCAN_COLOR
        : scanColor;
    ...
    const {
        ...
        hoverColor: iconHoverColor = "#00FF66",
        side: iconSide = "left",
        size: iconSize = 24,
    ...
    const resetToIdle = useCallback(() => {
        if (!scope.current) return;
        scope.current.style.setProperty("--scan-effect-color", scanColor);
    ...
    const runHover = useCallback(() => {
        ...
        scope.current.style.setProperty("--scan-effect-color", hoverScanColor);
        animate(
            scope.current,
        ...
        const filter = `drop-shadow(0px 0px 4px ${hoverScanColor})`;
        animate(".bracket-tl", { d: hoverPaths.tl, filter, stroke: hoverScanColor } as any, transition as any);
        animate(".bracket-tr", { d: hoverPaths.tr, filter, stroke: hoverScanColor } as any, transition as any);
        animate(".bracket-br", { d: hoverPaths.br, filter, stroke: hoverScanColor } as any, transition as any);
        animate(".bracket-bl", { d: hoverPaths.bl, filter, stroke: hoverScanColor } as any, transition as any);
    ...
    const runLeave = useCallback(() => {
        ...
        const filter = `drop-shadow(0px 0px 0px ${scanColor})`;
        animate(".bracket-tl", { d: idlePaths2.tl, filter, stroke: scanColor } as any, transition as any);
        animate(".bracket-tr", { d: idlePaths2.tr, filter, stroke: scanColor } as any, transition as any);
        animate(".bracket-br", { d: idlePaths2.br, filter, stroke: scanColor } as any, transition as any);
        animate(".bracket-bl", { d: idlePaths2.bl, filter, stroke: scanColor } as any, transition as any);
            ...
            >
                <div
                    style={{
                        position: "absolute",
                        inset: 0,
                        background: "linear-gradient(180deg, transparent 0%, color-mix(in srgb, var(--scan-effect-color) 45%, transparent) 100%)",
                    }}
                />
                <div
                    style={{
                        ...
                        width: "100%",
                        height: 2,
                        background: "var(--scan-effect-color)",
                        boxShadow: "0px 0px 6px 0px var(--scan-effect-color)",
                    }}
                />
            </div>
